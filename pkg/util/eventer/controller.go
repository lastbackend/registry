...
func (c *Controller) Add(id string, sock *socket.Socket) *Pool {
	log.V(logLevel).Debugf("%s:controller:add:> create new connections broker %s", logPrefix, id)

	var p = new(Pool)
	p.ID = id

	p.conns = make(map[*socket.Socket]bool)

	p.join = make(chan *socket.Socket)
	p.leave = make(chan *socket.Socket)
	p.close = c.leave
...
func (c *Controller) Attach(pool *Pool, sock *socket.Socket) {
...
func New() *Controller {
	...
	c.conns = make(map[*socket.Socket]*Pool, 0)
	c.join = make(chan *Pool)
	c.leave = make(chan *Pool)

	c.Message = make(chan *socket.Message)
	c.Leave = make(chan *socket.Socket)
